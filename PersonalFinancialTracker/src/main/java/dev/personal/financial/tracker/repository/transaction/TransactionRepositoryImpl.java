...
public class TransactionRepositoryImpl implements TransactionRepository {
    ...
    public void save(Transaction transaction) {
        String sql = "INSERT INTO app.transactions (user_id, amount, category, date, description, is_income)" +
                " VALUES (?, ?, ?, ?, ?, ?)";
    ...
    public List<Transaction> findByUserIdAndCategory(int userId, String category) {
        List<Transaction> transactions = new ArrayList<>();
        String sql = "SELECT * FROM app.transactions WHERE user_id = ? AND category = ?";
        try (PreparedStatement statement = connection.prepareStatement(sql)) {
            statement.setInt(1, userId);
    ...
    public List<Transaction> findByUserIdAndDate(int userId, LocalDate date) {
        List<Transaction> transactions = new ArrayList<>();
        String sql ="SELECT * FROM app.transactions WHERE user_id = ? AND date = ?";
        try (PreparedStatement statement = connection.prepareStatement(sql)) {
            statement.setInt(1, userId);
    ...
    public List<Transaction> findByUserIdAndType(int userId, boolean isIncome) {
        List<Transaction> transactions = new ArrayList<>();
        String sql = "SELECT * FROM app.transactions WHERE user_id = ? AND is_income = ?";
        try (PreparedStatement statement = connection.prepareStatement(sql)) {
            statement.setInt(1, userId);
    ...
    public BigDecimal getTotalExpensesForCurrentMonth(int userId) {
        String sql = "SELECT SUM(amount) FROM app.transactions WHERE user_id = ? AND is_income = false AND date >= ? AND date <= ?";
        try (PreparedStatement statement = connection.prepareStatement(sql)) {
    ...
    public List<Transaction> findByUserIdAndDateRange(int userId, LocalDate startDate, LocalDate endDate) {
        List<Transaction> transactions = new ArrayList<>();
        String sql = "SELECT * FROM app.transactions WHERE user_id = ? AND date >= ? AND date <= ?";
        try (PreparedStatement statement = connection.prepareStatement(sql)) {
            statement.setInt(1, userId);
